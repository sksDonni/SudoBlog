...
function Post() {
	...
	if(selectedPost == null)
	{
		...
		const postContentHtml = snarkdown(selectedPost.body)
	return (
		<div>
			<div className="w-1/2 m-auto">
				<h1 className="text-red-600 text-4xl font-semibold">{selectedPost.title}</h1>
				<h4>{new Date(selectedPost.createdAt).toLocaleString('en-IN').split(',')[0]}</h4>
				...
				<div className="w-1/4 m-auto my-5">
					<Link to={commentLink} className="text-center p-2 align-center border-red-200 border-2 text-md text-red-600 font-medium">View Comments</Link>
				</div>
			</div>
		</div>
	)
